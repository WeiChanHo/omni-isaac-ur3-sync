...
- 送出時在 Status 顯示目標關節、最大位移、速度與預估時間。
...
8. 按下 **Execute on Physical UR3**。Mock 模式中的按鈕仍會使用此名稱，
   ...
   立即送出，不會另外顯示確認視窗。
9. 確認 Status 短暫顯示命令摘要，接著顯示目標已接受與成功完成。
...
8. 在 Status 檢查目標關節、實機目前位置至目標的最大位移、速度及
   軌跡時間；此摘要會在 Action 狀態更新時被取代。

第一次測試時，最大關節位移不可超過 `0.05 rad`（約 `2.9 度`）。如果超過，
請使用 **Cancel Goal** 要求取消，並建立較小的目標。這是保守的
功能測試數值，不是經認證的
安全限制。
